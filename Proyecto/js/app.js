...
function cargarEventListeners() {
    ...
    // Ir el carrito de compras
    //id del boton proceder para ver carrito
...
// Define la función que redirecciona al usuario
function verCarrito() {
    // Redirecciona al usuario a la página 'carrito.html'
...
function actualizarSubtotal() {
    var productos = document.querySelectorAll('#lista-productos tbody tr');
    var total = 0;

    productos.forEach(function(row) {
        var precio = parseFloat(row.cells[2].textContent); // Asume que el precio está en la tercera columna
        var cantidad = parseInt(row.cells[3].children[0].value); // Asume que la cantidad es un input en la cuarta columna
        total += precio * cantidad;
    });

    document.getElementById('subtotal-valor').textContent = total.toFixed(2);
}

// Ejemplo de cómo podrías llamar esta función
// Esto deberías hacerlo cada vez que se añade, elimina un producto o se cambia una cantidad
actualizarSubtotal();
